...
class CustomAlbert {
  ...
  init(weights: [String: MLXArray], config: AlbertModelArgs) {
    ...
    if let poolerWeight = weights["bert.pooler.weight"], let poolerBias = weights["bert.pooler.bias"] {
  ...
  func callAsFunction(
    ...
    let pooledOutput = pooler.map { MLX.tanh($0(firstTokenReshaped)) } ?? firstTokenReshaped
